%Macro HashLookup(BaseDS=,LookupDS=,Key=NIL,Values=NIL,Filter=NIL,BFilter=NIL)/Store Secure ;

%Put INFO: HashLookup macro executing !!!;
...
%If %CMPRES(&Values) eq %STR()  %Then %Do ;
    ...
    %GOTO EXIT ;
%End ;
%Else %Put MACROMSG: HashLookup Macro will Lookup all the variables from &Lib..&DS. Dataset !!! ;
...
/*	%Put rValues=&rValues ;*/
/*	%Put Scan=%Scan(&Values,&i,' ') ;*/
/*	%Put ifc=%Sysfunc(ifc(%Cmpres(&Type) eq $,%Sysfunc(Compress(&Type.&Lngth.)),8.)) ;*/
    %Let Length=&Length %Cmpres(%Scan(&Values,&i,' ')) %Sysfunc(ifc(%Cmpres(&Type) eq $,%Sysfunc(Compress(&Type.&Lngth.)),8.)) ;
    %Let Sep=, ;
%End ;
...
%If %Bquote(&BFilter.) NE %Bquote(NIL) %Then %Do ;
    BFilter=(%Sysfunc(Dequote(&BFilter.)));
%End ;
%Else %Do ; 
    BFilter=1;
%End ;

if h1.check() eq 0 and BFilter eq 1 then do ;
h1.find() ;
end ;
Drop BFilter ;
Run ;
...
%EXIT:
%Put Info: Hashlookup Macro Executed !!! ;
